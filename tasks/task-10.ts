...
const cartInfo = {
    product: (productCode: string, productName: string, price: number, quantity: number, isFreeShipping: boolean) => ({
        productCode, productName, price, quantity, isFreeShipping
    })
};

const cartData = [
    cartInfo.product("KB001", "Mechanical Keyboard", 850000, 1, true),
    cartInfo.product("MS002", "Wireless Mouse", 275000, 2, true),
    cartInfo.product("HD003", "External SSD 1TB", 1650000, 1, false),
    cartInfo.product("HP004", "Gaming Headset", 720000, 1, true)
];
